...
public class LoadMyPhotoTask extends AsyncTask<Integer, Void, List<MyPhoto>> {

    private WallSplashDatabase db;
    private OnPostLoadListener listener;

    public LoadMyPhotoTask(WallSplashDatabase db, OnPostLoadListener l) {
        this.db = db;
        this.listener = l;
    }
    ...
    protected List<MyPhoto> doInBackground(Integer... integers) {
        return db.myPhotoDao().getAll(integers[0]);
    }
